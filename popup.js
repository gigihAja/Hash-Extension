...
document.getElementById("lookupBtn").addEventListener("click", async () => {
    ...
    if (ipList.length > 0) {
        ...
        for (const ip of ipList) {
            const container = vt.querySelector(".content");
            await lookupVirusTotalIP_V3(ip, container);
            await lookupAbuseIPDB(ip, container);
        }
...
async function lookupAbuseIPDB(ip, container) {
    try {
